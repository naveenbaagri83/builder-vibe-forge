...
function buildNASAAnswer(query: string, docs: SourceDoc[]): ChatResponse {
  ...
  const points = top
    .map(
      (d, i) =>
        `(${i + 1}) ${d.title}${d.mission ? ` — ${d.mission}` : ""}${d.organism ? ` — ${d.organism}` : ""}`,
    )
    .join("\n");
...
export const handleChat: RequestHandler = async (req, res) => {
  try {
    ...
    const resp: ChatResponse = {
      shortSummary: text
        ? text.split("\n").slice(0, 2).join(" ")
        : `No NASA records found for "${body.query}" and Gemini fallback unavailable.`,
      detailedAnswer:
        text ||
        "Gemini API key not configured. Please set GEMINI_API_KEY to enable general Q&A.",
      sources: [],
      followUps: [
